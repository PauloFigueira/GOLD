...
var Ball = EXTENDS(Actor, {
	...
	collision: function(hit) {
		...
		if (hit.killer()) { this.dying=true;

			GameControl.animationEvent();
		}


		if(hit.bucketON()) {
		this.setColor(hit.getColor());	
		}

		if (hit.consumable()) {
			if (hit.getColor()==this.getColor() || hit.GoldON() ){
				this.nBrick++;
				console.log(this.nBrick);
				hit.hide();
				this.points+= hit.pts();
				var score = document.getElementById('pontos');
				score.innerHTML = parseInt(score.innerHTML,10) + hit.pts();
			}
		}
	}
})
...
function onLoad() {
  // load images an then run the game
	loadImages(function() {NEW(GameControl);});
